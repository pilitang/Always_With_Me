...
class_name CommmonHFlowContainer
...
func _calculate_layout(apply: bool) -> float:
	...
	var minWidth = container_width/columns
	for child in get_children():
		if not child.has_method("get_combined_minimum_size"):
			continue
		...
		if child_min_size.x + horizontal_margin < minWidth:
			child_min_size =Vector2(minWidth-horizontal_margin, child_min_size.y)
			child.custom_minimum_size = child_min_size
